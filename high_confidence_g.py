...
def main():
    ...
    if not args.no_graph:
        ...
        # also drop a copy in the root dir, named "<current directory>_YYYY_MM_DD_G.html"
        # (illegal filename characters underscored so the name is always valid).
        # "_G" suffix keeps this distinct from high_confidence.py's "<dir>_<date>.html" copy.
        dirname = re.sub(r'[\\/:*?"<>|\s]+', "_", ROOT.name)
        today = datetime.date.today().strftime("%Y_%m_%d")
        dest = ROOT / f"{dirname}_{today}_G.html"
